  ...
  lateHook("data-icon", (el, name) => {
    ...
    if ([...el.children].some((c) => String(c.className || "").split(" ").includes("wf-icon"))) return;
    // A name that reads state follows it: `icon: if dark { "sun" } else { "moon" }`.
    if (typeof name === "function") effect(() => _renderIcon(el, name()));
    else _renderIcon(el, name);
  });
