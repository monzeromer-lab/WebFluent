...
fn apply(s: &mut StyleProps, name: &str, value: &Expr) {
    match name {
        ...
        "font-size" => {
            if let Some(v) = expr_dimension_pt(value) {
                s.font_size = Some(v);
            }
        }
        "color" => {
            if let Some(v) = expr_color(value) {
                s.color = Some(v);
            }
        }
        "text-align" => {
        ...
        // ─── Dimensions ─────────────────────────────────────
        "width" => {
            s.width = expr_dimension(value);
        }
        "height" => {
            s.height = expr_dimension(value);
        }
        ...
        "border-color" => {
            if let Some(v) = expr_str(value).and_then(|v| parse_color(&v)) {
                s.border_color = Some(v);
            }
        }
        "border-width" => {
            if let Some(v) = expr_dimension_pt(value) {
                s.border_width = Some(v);
            }
        }
        "border-radius" => {
            if let Some(v) = expr_dimension_pt(value) {
                s.border_radius = Some(v);
            }
        }
        ...
        "transition" | "animation" | "transform" | "opacity" | "display" | "position"
        | "z-index" | "overflow" => {}
...
fn parse_dimension_pt(s: &str) -> Option<f64> {
    ...
    if let Some(n) = s.strip_suffix("em") {
        return n.trim().parse::<f64>().ok().map(|v| v * 12.0);
    }
    s.parse().ok()
}
