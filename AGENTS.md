...
| Data | `resource(url, options)` → `{state, data, error, reload}`; `request(url, options)` → the parsed body (`await fetch(…)` in an action compiles to it); `fetch(url, options)`; `store(def)`; `emit(props, event, …args)`; `locales(default, tables)` → `WF.i18n` |
