...
| 2026-09-26 | 3f. Offline and peers (4.1) | **Done**, with what had to come first. **The editors**: the Zed grammar marked every v4-only construct an error — `api`, connections, `validate`, refined types, the language's literals — and failed on any call with a bare-name argument (`remove(i)`), read as a scoped slot's fill; it reads all of it, held to every compiler-accepted block of the guide (269, in `test/guide`, kept current by a test), and VS Code 0.3.0 is bundled (0.2.0 could not start), downloads `wf-lsp`, and is held to Zed's vocabulary. **Releases** are one tag: a preflight (versions, notes, the Zed pin — it fails on the stale pin 4.0.1 shipped), the release body from the notes, the `.vsix` attached, and `cargo publish` and the marketplaces when their secrets exist. **Build and runtime agree**: 61 samples compare each method's build-time value with the runtime's; it found `text.lines()` returning `undefined` wherever the network module was built in (two modules declaring `lines` in one closure) and `url.query()`/`url.with()` painting nothing. **Offline**: `sw.js` versioned by a hash of the build, precache by route globs, a fallback by redirect, cache policies, `update.available`/`update.apply()` reloading once, `sync` keeping writes in IndexedDB and sending them under a Web Lock from page and worker, `network.queued`. **Peers**: `peer link = rtc(signal: …)`, a socket's handle over a WebRTC data channel, signalled over the app's own transport. Both are held to a real Chrome in CI (`tests/browser`, 28 checks). Found on the way: text reading a browser value (`viewport`, `now`, `network`…) was computed once and never moved; a splice's own string lost its escapes, and `{a ?? \"x\"}` was shown as text; a peer that left read `error`, and the other side waited 30 s to see it; a `site_url` with the subpath doubled it in every canonical URL. Not in this repository: the studio integration, which the plan leaves to `wf-studio` and its own plan. |
